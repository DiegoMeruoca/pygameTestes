...
# Função que controla a captura de enentos
def controlador_eventos_mapa(personagem, controle):
    for evento in pygame.event.get():  # event loop
        if evento.type == QUIT:  # Se o evento for QUIT (sair)
            pygame.quit()  # Finaliza o game e fecha a tela
            sys.exit()  # Fecha o programa
        ...
        if evento.type == KEYDOWN:  # Ao pressionar alguma tecla
            if evento.key == controle.direita:  # Se for direita
                personagem.andando_dir = True  # Andar pra direita passa a ser verdadeiro
            if evento.key == controle.esquerda:
                personagem.andando_esq = True
            if evento.key == controle.pulo:
                if personagem.tempo_no_ar < 6:  # Para um double jump experimente 30
                    personagem.personagem_y_momentum = -5
                    personagem.pulando = True
                    personagem.som_pulo.play()

        if evento.type == KEYUP:  # Ao soltar alguma tecla
            if evento.key == controle.direita:  # Se for seta direita
                personagem.andando_dir = False  # Andar direita passa a ser falso
            if evento.key == controle.esquerda:
                personagem.andando_esq = False
...
controle1 = Controle(K_RIGHT, K_LEFT, K_UP)
controle2 = Controle(K_d, K_a, K_w)
...
while True:  # Lop infinito do game
    ...
    if tela1:
        mapa1.rolagem_tela = controle_rolagem_tela(personagem1)  # Cahama o controle de camera
        ...
        controlador_eventos_mapa(personagem1, controle2)  # Cama a função para capturas e controle de eventos
